...
public class FileHelper {
	...
	public static boolean fileIsExist(String filePath) {
		if (filePath == null || filePath.length() < 1) {
			System.out.println("param invalid, filePath: " + filePath);
			return false;
		}
	...
	public static InputStream readFile(String filePath) {
		if (null == filePath) {
			System.out.println("Invalid param. filePath: " + filePath);
			return null;
		}
	...
	public static boolean deleteDirectory(String filePath) {
		...
		if (file.isDirectory()) {
			...
			for (int i = 0; i < list.length; i++) {
				System.out.println("delete filePath: " + list[i].getAbsolutePath());
	...
	public static boolean writeFile(String filePath, InputStream inputStream) {
		...
		try {
			  ...
			  if (file.exists()) {
		       ...
		       if (!ret){
		    	   System.out.println("createDirectory fail path = " + pth);
		    	   return false;
		       }
		       
		       boolean ret1 = file.createNewFile();
		       if (!ret){
		    	   System.out.println("createNewFile fail filePath = " + filePath);
		    	   return false;
		       } 
	...
	public static boolean writeFile(String filePath, String fileContent, boolean append) {
		if (null == filePath || fileContent == null || filePath.length() < 1 || fileContent.length() < 1) {
			System.out.println("Invalid param. filePath: " + filePath + ", fileContent: " + fileContent);
			return false;
		}
		...
		try {
	       ...
	       output.close();
			...
			System.out.println("writeFile ioe: " + ioe.toString());
			return false;
		}
	...
	public static boolean copyFile(String fromPath, String destUri) {
		if (null == fromPath || fromPath.length() < 1 || null == destUri || destUri.length() < 1) {
			System.out.println("copyFile Invalid param. fromPath="+fromPath+", destUri="+destUri);
			return false;
		}
		...
		try {
			...
			if (null == is) {
				System.out.println("Failed to open inputStream: "+fromPath+"->"+destUri);
				return false;
			}
			...
			if (null != path) {
	            ...
	            if (!pf.exists()) {
	                if (!pf.mkdirs()) {
	                    System.out.println("Can't make dirs, path=" + pth);
	                }
	            }
	...
	public static boolean writeFile(String filePath, byte[] content) {
		if(null == filePath || null == content) {
			System.out.println("Invalid param. filePath: "+filePath+", content: "+content);
			return false;
		}
	...
	public static byte[] readGZipFile (String zipFileName) {
		if (fileIsExist(zipFileName)) {
			...
			try {
				...
				System.out.println("read zipRecorder file error");
			}
		}
		return null;
	}
	...
	public static boolean unZipFile(String fileName, String unZipDir) throws Exception {
		...
		while (enumeration.hasMoreElements()) {
			...
			if (entry.isDirectory()) {
				...
				System.out.println("entry.isDirectory XXX " + f1.getPath());
				if (!f1.exists()) {
					f1.mkdirs();
				}
			} else {
